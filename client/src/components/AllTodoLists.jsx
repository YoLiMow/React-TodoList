    ...
    getAllLists(user)
      .then((listsData) => {
        ...
        if (listsData.length > 1 && !listsData[0]?.isAllTodos) {
          ...
          const index = oldListsArray.findIndex(
            (li) => li.id === allTodosListId
          );
          const list = { ...oldListsArray[index] };

          // remove list from previous position in the array
          const filteredArray = oldListsArray.filter(
            (li) => li.id !== allTodosListId
          );
        ...
        {data != null &&
          data.map(({ name, id, icon }) => {
            ...
            if (!isAllTodosList(id)) {
                      ...
                      sx={
                        id === currentList
                          ? {
                              color: themeSelection.palette.secondary.main,
                              backgroundColor:
                                themeSelection.palette.primary.main,
                              fontWeight: "bold",
                            }
                          : {}
                      }
              ...
              return <AllTodosListItem key={id} id={id} name={name} />;
            }
          })}
      </List>
    </Drawer>
