...
export function CurrentTodoList({
  ...
  useEffect(() => {
    async function getInitialData() {
      try {
        ...
        const li = await getAllTodosListItems(user);
        setIsListEmpty(li == null || li.items.length === 0);
        setData(li);
        ...
        li?.items.map((item) =>
          setIsEdit((prev) => [
            ...prev,
            { id: item.id, priorityEdit: false, dateEdit: false },
          ])
        );
      } catch (err) {
        console.log(err);
  ...
  function handleUpdateItem(updatedItem, priority) {
    ...
    const index = itemsArray.findIndex((i) => i._id === updatedItem.id);
    const newItemsArray = [...itemsArray];
    newItemsArray[index].name = updatedItem.name;
    if (priority != null) {
      newItemsArray[index].priority = updatedItem.priority;
    }
    setData((prev) => ({
      ...prev,
      items: newItemsArray.sort((a, b) => sortItems(a, b, sortSelection)),
    }));
  }
  ...
  async function handleToggleChecked(id) {
    ...
    return await toggleChecked(id, itemsArray[index].checked);
  }

  // manages list name change
  function onListUpdate(event) {
    setData((prev) => ({ ...prev, name: event.target.value }));
    // update lists in AllTodoLists
    handleListUpdated();
    // update list in db
  ...
  const onChangeUpdateItemPriority = async (event, id) => {
    try {
      ...
      newItemsArray[index].priority = event.target.value;
      setData((prev) => ({
        ...prev,
        items: newItemsArray.sort((a, b) => sortItems(a, b, sortSelection)),
      }));
  ...
  function toggleIsPriorityEdit(id) {
    const index = getEditId(id);
    const newIsEdit = [...isEdit];
    newIsEdit[index].priorityEdit = !newIsEdit[index].priorityEdit;
    const newData = { ...data };
    setData(newData);
  }
  ...
  function toggleIsDateEdit(id) {
    const index = getEditId(id);
    const newIsEdit = [...isEdit];
    newIsEdit[index].dateEdit = !newIsEdit[index].dateEdit;
  ...
  const onChangeSort = (event) => {
    ...
    if (newDataItems != null) {
      const sortedDataItems = newDataItems.sort((a, b) =>
        sortItems(a, b, event.target.value)
      );
  ...
  const onChangeTodayItems = async (event) => {
    ...
    if (event.target.checked && data.items != null) {
      ...
      const newDataItems = data.items.filter(
        (item) =>
        ...
        {data ? (
          <>
            ...
            {!isListEmpty ? (
              ...
              <Typography>The selected list has no items.</Typography>
            )}
            ...
            >
              {data.items != null &&
                data.items.map(({ id, name, checked, priority }) => {
                        ...
                        <ListItemText id={labelId}>
                          <TextField
                            ...
                            onChange={(event) => {
                              ...
                              listItems[index].name = event.target.value;
                              setData((prev) => ({
                                ...prev,
                                items: listItems,
                              }));
